...
O **Projeto Farmacia Cura Tudo** é uma aplicação que permite que usuários cadastrar, editar e visualizem produtos com categorias variadas, de forma organizada e segura. Este projeto foi desenvolvido com fins educacionais, simulando uma aplicação real de um e-commerce de farmacia para praticar conceitos de API REST com Java e Spring Boot.
 
Entre os principais recursos que um blog pessoal oferece, destacam-se:
 
1. Criação, edição e exclusão de produtos
2. Associação de produto a categorias específicas
...
A API da Farmacia Cura Tudo foi desenvolvida utilizando **Java** e o **framework Spring**, seguindo os princípios da Arquitetura MVC e REST. Ela oferece endpoints para o gerenciamento dos recursos **Usuário**, **Produto** e **Categoria**, permitindo a visualização do usuários aos produtos à venda.
...
2. Consulta, criação e gerenciamento de categorias para classificar produto
3. Criação, edição, listagem e remoção de produto
4. Associação de produto a categorias
...
O **Diagrama de Classes** é um modelo visual usado na programação orientada a objetos para representar a estrutura de um siscategoria. Ele exibe classes, atributos, métodos e os relacionamentos entre elas, como associações, heranças e dependências.
 
Esse diagrama ajuda a planejar e entender a arquitetura do siscategoria, mostrando como as entidades interagem e se conectam. É amplamente utilizado nas fases de design e documentação de projetos.
...
erDiagram
    ...
    tb_categorias ||--o{ tb_produto : classifica
    tb_usuarios {
        bigint id PK
        varchar(255) nome
        varchar(255) usuario
...
```bash
git clone https://github.com/jmcardoso18/projeto_final_bloco_02
```
